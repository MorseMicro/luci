...
var CBIAbstractSection = CBIAbstractElement.extend(/** @lends LuCI.form.AbstractSection.prototype */ {
	...
	formvalue: function(section_id, option) {
		...
		for (var i = 0, o; (o = this.children[i]) != null; i++) {
			var func = (this.map.root && this.map.root.children.length > 0) ? this.children[i].formvalue : this.children[i].cfgvalue;
...
var CBIValue = CBIAbstractValue.extend(/** @lends LuCI.form.Value.prototype */ {
	 ...
	 *
	 * NB: this will cause onchange handlers to be called, even if the underlying value remains the same.
	 *
	 * @param {string} section_id
	 * @param {string} new_value optionally set a new value
	 */
	renderUpdate: function(section_id, new_value) {
		...
		var cbid = this.cbid(section_id),
			map = this.map,
			oldOptionEl = map.findElement('data-field', cbid),
			option_index = oldOptionEl.dataset.index,
			in_table = oldOptionEl.parentElement?.matches('td.cbi-value-field') ?? false;

		return Promise.resolve(this.renderWidget(section_id, option_index, new_value ?? this.formvalue(section_id)))
			.then(this.renderFrame.bind(this, section_id, in_table, option_index))
			.then(function (optionEl) {
				...
				// and replace it).
				var oldOptionEl = map.findElement('data-field', cbid);
