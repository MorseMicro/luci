...
var CBIAbstractValue = CBIAbstractElement.extend(/** @lends LuCI.form.AbstractValue.prototype */ {
	...
	parse: function(section_id) {
		...
		if (active && !this.isValid(section_id)) {
			var title = this.stripTags(this.displayname || this.title).trim(),
...
var CBIValue = CBIAbstractValue.extend(/** @lends LuCI.form.Value.prototype */ {
	...
	/** @override */
	render: async function(option_index, section_id, in_table) {
		const cfg_value = await this.cfgvalue(section_id);
		const nodes = await this.renderWidget(section_id, option_index, cfg_value);
		return await this.renderFrame(section_id, in_table, option_index, nodes, cfg_value);
	},
	...
	handleValueChange: function(section_id, state, ev) {
		...
		var previousValue = state.previousValue;

		if (isEqual(value, previousValue))
			return;

		state.previousValue = value;
		this.onchange.call(this, ev, section_id, value, previousValue);
	},

	/** @private */
	renderFrame: function(section_id, in_table, option_index, nodes, old_value) {
		var config_name = this.uciconfig || this.section.uciconfig || this.map.config,
		...
		optionEl.addEventListener('widget-change',
			L.bind(this.handleValueChange, this, section_id, { previousValue: old_value }));
	...
	/**
	 * Clear current form data. .renderUpdate() should be called after new .value() elements are added.
	 *
	 * For a justification for this, see .renderUpdate().
	 */
	clear: function() {
	...
	/**
	 * Re-render widget with current value() data.
	 *
	 * This has been added because:
	 *  - LuCI only supports rerendering the entire map, which can cause
	 *    a 'flash' on the screen and is an unpleasant amount of work for a minor change
	 *  - it's dangerous to try to reach into the UIElements to change their underlying
	 *    data, as this can mess up the stateful interactions (incl caching) between
	 *    the form.Value and the UIElement
	 *  - it's annoying to completely bypass the UIElement logic (cf CBIWifiFrequencyValue)
	 *    and handle your own rendering entirely, as you end up reimplementing
	 *    (or half-implementing) the underlying widgets
	 *
	 * @param {string} section_id
	 * @param {string} new_value optionally set a new value
	 */
	renderUpdate: async function(section_id, new_value) {
		// As far as I can determine, there's no nice way to get the original optionEl
		// generated by renderFrame which we want to replace, so we do an id lookup.
		const cbid = this.cbid(section_id);
		let oldOptionEl = this.map.findElement('data-field', cbid);
		const in_table = oldOptionEl.parentElement?.matches('td.cbi-value-field') ?? false;
		const option_index = oldOptionEl.dataset.index;

		const old_value = this.formvalue(section_id);
		const nodes = await this.renderWidget(section_id, option_index, new_value ?? old_value);
		// We pass old value to render frame so the event dispatch works (see widget-change below).
		const optionEl = await this.renderFrame(section_id, in_table, option_index, nodes, old_value);

		// We need to re-request the optionEl because multiple changes
		// might be trying to trigger a change to this element, in which
		// case oldOptionEl might be deleted before we get here
		// (since we're in a promise). But it's fine to pick up the new one
		// and replace it).
		oldOptionEl = this.map.findElement('data-field', this.cbid(section_id));
		...
		// Even if a new value wasn't set, we should trigger widget change in case
		// the old value is no longer a valid option (e.g. <select> has changed).
