...
var CBIWifiFrequencyValue = form.Value.extend({
	...
	getCurrentHalowChannels: function(elem) {
		return this.halowChannels.getMap(
			...
			elem.querySelector('.s1g-chzn').value,
		);
	},
			...
			if (uci.get('wireless', section_id, 'type') == 'morse') {
				...
				if (activeCountry) {
					...
					this.halowChannels.restrict(
						activeCountry.code, uci.get('wireless', section_id, 's1g_chzn'), activeChannels
					);
				}
				return;
			}
	...
	updateS1gChannelization: function(elem, initialVal) {
		var s1gChznEl = elem.querySelector('.s1g-chzn');
		var s1gChznValues = [];
	...
	setInitialValues: function(section_id, elem) {
		var mode = elem.querySelector('.mode'),
		    ...
		    s1gchzn = elem.querySelector('.s1g-chzn'),
		...
		if (type === 'morse') {
			if (!chznval) {
				chznval = this.halowChannels.getDefaultChannelization(country)?.codename;
			}
	...
	formvalue: function(section_id) {
		...
		return [
		    ...
		    node.querySelector('.s1g-chzn').value || undefined,
	...
	write: function(section_id, value) {
		...
		if (this.isNativeS1G(section_id)) {
			if (value[0] !== undefined) {
				uci.set('wireless', section_id, 's1g_oper_chwidth', value[0]);
			}
		}
		else
		...
		uci.set('wireless', section_id, 's1g_chzn', value[6]);
	},
